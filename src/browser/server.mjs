...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/api/jev/settings' && request.method === 'GET') return replyJson(200, runtime.jevStatus())
    if (url.pathname === '/api/jev/settings' && request.method === 'POST') {
      const body = await bodyOf(request, 20_000)
      if (!body || Object.keys(body).join(',') !== 'apiKey') return replyJson(400, { error: 'JEV_KEY_INVALID' })
